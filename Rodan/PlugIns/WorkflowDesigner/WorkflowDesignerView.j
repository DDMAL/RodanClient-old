...
/* ------------------------------ CODE TO BE AUTOMATED VIA DRAP & DROP ---------------------------- */
...
/* --------------------------------------- AUTOMATED CODE END --------------------------- */
    ...
    outputPortView = [[CPView alloc] initWithFrame:CGRectMake(0, 0, 200, 200)];
    ...
    var outputTypeText = [[CPTextField alloc] initWithFrame:CGRectMake(10, 10, 70, 30)];
    [outputTypeText setStringValue:@"Output Type:"];
    [outputPortView addSubview:outputTypeText];


    inputPortView = [[CPView alloc] initWithFrame:CGRectMake(0, 0, 200, 200)];
    ...
    var inputTypeText = [[CPTextField alloc] initWithFrame:CGRectMake(10, 10, 70, 30)];
...
- (void)receiveDragLink:(CPNotification)aNotification
{
    var info = [aNotification userInfo],
        ...
        k,
        currentMouseLocation = [self convertPoint:mouseLocation fromView:nil];
        //temporary bug fix for convertPoint
        currentMouseLocation.y = currentMouseLocation.y - 30;
...
- (void)receiveOutputEntered:(CPNotification)aNotification
{
    ...
    var info = [aNotification userInfo],
        // mouseLocation = [self convertPoint:[CPEvent mouseLocation] fromView:nil],
        mouseLocation = [self convertPointFromBase:[CPEvent mouseLocation]],


        workflowNumber = [info objectForKey:"workflow_number"],
        outputNumber = [info objectForKey:"output_number"],
        linkIndex = workflowJobs[workflowNumber].outputPorts[outputNumber].linkIndex;
...
- (void)receiveInputEntered:(CPNotification)aNotification
{
     var info = [aNotification userInfo],
        ...
        linkIndex = workflowJobs[workflowNumber].inputPorts[inputNumber].linkIndex;

    // console.log("Entered Output");

    [inputPortView setHidden:NO];
    [inputPortView setFrameOrigin:CGPointMake(mouseLocation.x - 200, mouseLocation.y)];
